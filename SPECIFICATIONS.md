...
- **Subject selector**: required to start either mode.
  - **Switch to Pomodoro**: when running in Simple mode, a "Switch → 🍅" button appears (when Pomodoro is enabled). Clicking it computes the elapsed time, walks the Pomodoro cycle saving any completed focus blocks as sessions, and enters the current phase at the correct offset — as if Pomodoro had been running from the start. The timer's current-session group is preserved.
...
- **Default workflow:** after `npx tsc --noEmit`, `npx vitest run`, and `npm run build` all pass, commit the changes, push to `org/main`, then run `npm run deploy`. Do not stop after a green build — the live site is the deliverable.
  ...
  - **Report the app version on every live deploy:** after `npm run deploy` succeeds, the assistant MUST state the deployed app version number (the `VERSION` from `package.json` / `src/lib/version.ts`, e.g. `v0.22.4`) in the completion message. Do not report "deployed" without naming the version — the version is how the user confirms the live site matches the change.
...
**SPEC_VERSION: 45** — 2026-08-30 Added §13 rule: after `npm run deploy` succeeds, the assistant MUST state the deployed app version (e.g. `v0.22.4`) in the completion message. Do not report "deployed" without naming the version — the version is how the user confirms the live site matches the change.
...
**SPEC_VERSION: 46** — 2026-08-30 Seven UX bug fixes: (1) AI Review sticky bottom action bar converted to normal-flow so it can no longer obscure the sidebar or page content (§13). (2) Dashboard header button label renamed from "Customize" to "Customise" (Australian English, §7). (3) Dashboard "Last 7 days" widget no longer renders its date range twice (§6.1). (4) Dashboard Recent Sessions now includes time logged under all focus areas, not academic-only (§6.1). (5) Streak heatmap "next goal" pulse animation restricted to the day before and the day of goal completion, not continuous (§6.1). (6) Compact schedule mode no longer requires avoidable scrolling (§6.6). (7) Study timer surfaces the selected routine being studied for while running, with an explicit "No routine" empty state (§6.8).
**SPEC_VERSION: 47** — 2026-09-11 Added "Switch to Pomodoro" feature: when running in Simple mode, a "Switch → 🍅" button appears (when Pomodoro is enabled). Clicking computes elapsed time, walks the Pomodoro cycle saving completed focus blocks as sessions, and enters the current phase at the correct offset — as if Pomodoro had been running from the start. The timer's current-session group is preserved.
**SPEC_VERSION: 48** — 2026-09-11 Fixed Simple→Pomodoro accounting: persist the backdated current-phase start so Today totals, Today by Subject, and heatmaps retain pre-switch time; avoid double-counting that phase in the current-session group; allow active timer controls to wrap within narrow widgets.
**SPEC_VERSION: 49** — 2026-09-12 Added a one-time Pomodoro first-focus duration override. While idle in Pomodoro mode, users may enter a custom focus duration; it applies only to the first focus phase of the next session, survives refresh during that phase, and then subsequent cycles use the configured duration.
**SPEC_VERSION: 50** — 2026-09-12 Dashboard improvements: (1) Today's Checklist supports adding today-dated tasks inline (title + focus area form at the list bottom; tasks toggle complete directly, no skip). (2) Study Streak widget shows a live "N days to next milestone" label that advances as the streak grows (§6.1). (3) Customise Dashboard rows now use proper dark-mode text colors (§7). (4) Project page completed-tasks disclosure replaced the ▼ emoji with a themed chevron SVG. (5) Dashboard FAB simplified to a single Log-Study-Time action (removed unused quick-add entries for pomodoro/marks/tasks/subjects).
**SPEC_VERSION: 51** — 2026-09-14 Fixed activity catch-up to create a Session on the correct date. When confirming an activity as done via the catch-up prompt ("Did you complete X on Y?"), the activity now creates a Session with `startAt` anchored to the missed date instead of silently logging only the ActivityLog. The session is linked via `sessionId` on the ActivityLog, and undo/redo handle both. This mirrors the routine catch-up behavior which already created sessions on the correct date. Fixes the issue where logging an activity late didn't add the study time to the day it was done.
**SPEC_VERSION: 52** — 2026-09-15 Fixed three bugs in the current-session group tracker: (1) `resetPomodoro()` (Pomodoro Save) never called `updateSessionGroup()`, so the "Current Session" display reset to 0 after saving in Pomodoro mode — fixed by updating the group with `active: false` and `lastEndAt: now`. (2) `changeSubject()` in simple mode set `lastSavedCumulativeRef.current = elapsed` BEFORE the `updateSessionGroup()` bump call, so the delta was `elapsed - elapsed = 0` — the old subject's live time was never added to the group. Fixed by computing delta before the ref write. (3) `stopSimple()` set `lastSavedCumulativeRef.current = total` AFTER the bump, so on the second stop the delta excluded the live seconds. Fixed by computing delta before the ref write so subsequent stops correctly carry only the unsaved delta.